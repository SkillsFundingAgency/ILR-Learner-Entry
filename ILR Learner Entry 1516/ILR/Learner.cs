...
namespace ILR
{
    public class Learner : ChildEntity, IDataErrorInfo
    {
        ...
        public bool HasContinuingAims
        {
            get
            {
                return LearningDeliveriesToMigrate.Count > 0;
            }
        }
                    ...
                    switch (learningDelivery.AimType)
                    {
                        ...
                        case 3:
                            if(learningDelivery.ShouldProbablyMigrate)
                                ...
                                learningDeliveriesToMigrate.Add(learningDelivery);
                            else if (learningDelivery.ProgType == 25 && learningDelivery.TBS!=null && this.LearningDeliveryList.Exists(ld => ld.AimType == 1 && ld.ShouldProbablyMigrate && ld.ProgType == learningDelivery.ProgType))
                                learningDeliveriesToMigrate.Add(learningDelivery);
                                break;
                    }
                }
        ...
        internal Learner(Learner MigrationLearner, XmlNode LearnerNode, XmlNamespaceManager NSMgr)
        {
            ...
            {
                if (!LLDDandHealthProblemList.Exists(l => l.LLDDCat == migrationItem.Get1516Equivalent()))
                {
                    XmlNode newNode = Node.OwnerDocument.CreateElement("LLDDandHealthProblem", NSMgr.LookupNamespace("ia"));
            ...
            if (LLDDandHealthProblemList.Count == 1)
                LLDDandHealthProblemList.First().PrimaryLLDD = true;

            foreach (LearnerFAM migrationItem in MigrationLearner.LearnerFAMList)
            {
                if (migrationItem.LearnFAMType != "HNS" && migrationItem.LearnFAMType != "LSR" && migrationItem.LearnFAMType != "FMR" && migrationItem.LearnFAMType != "PPE" && (migrationItem.LearnFAMType != "NLM" || (migrationItem.LearnFAMCode != 19 && migrationItem.LearnFAMCode != 20)) && (migrationItem.LearnFAMType != "MGA" || migrationItem.LearnFAMCode != 1) && (migrationItem.LearnFAMType != "EGA" || migrationItem.LearnFAMCode != 1))
                {
                    XmlNode newNode = Node.OwnerDocument.CreateElement("LearnerFAM", NSMgr.LookupNamespace("ia"));
                ...
                switch (columnName)
                {
                    ...
                    case "TelNumber":
                        if (this.TelNumber != null)
                        {
                            ...
                            Int64 x;
                            if (!Int64.TryParse(this.TelNumber, out x))
