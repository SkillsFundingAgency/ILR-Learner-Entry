...
namespace ILR
{
    public class LearnerEmploymentStatus : ChildEntity, IDataErrorInfo
    {
        ...
        public bool RON
        {
            get
            {
                return GetESMCode(EmploymentStatusMonitoring.ESMTypes.RON) == 1;
            }
            set
            {
                if (value)
                    SetESM(EmploymentStatusMonitoring.ESMTypes.RON, 1);
                else
                    RemoveESM(EmploymentStatusMonitoring.ESMTypes.RON);
                OnPropertyChanged("RON");
            }
        }
        public bool SEM
        {
