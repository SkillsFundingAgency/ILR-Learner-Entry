...
namespace ILR
{
	public class LearningDelivery : ChildEntity, IDataErrorInfo
	{
		...
		/// </summary>
		protected void OnLearningDeliveryPropertyChanged()
		{
		...
		private Boolean IsImportRunning { get; set; }
		public override bool IsComplete
		{
			get
			{
				if (this.LearnAimRef == null || this.AimType == null || this.AimSeqNumber == null || this.LearnStartDate == null || this.LearnPlanEndDate == null || this.FundModel == null || this.CompStatus == null)
		...
		public override string IncompleteMessage
		{
			...
			{
				string message = "";
				if (this.LearnAimRef == null)
					message += "\t\tLearnAimRef missing\r\n";
				if (this.AimType == null)
					message += "\t\tAimType missing\r\n";
				if (this.AimSeqNumber == null)
					message += "\t\tAimSeqNumber missing\r\n";
				if (this.LearnStartDate == null)
					message += "\t\tLearnStartDate missing\r\n";
				if (this.LearnPlanEndDate == null)
					message += "\t\tLearnPlanEndDate missing\r\n";
				if (this.FundModel == null)
					message += "\t\tFundModel missing\r\n";
				if (this.CompStatus == null)
					message += "\t\tCompStatus missing\r\n";
				return message;
			}
		}
		#endregion
		...
		public string ESFProjDosNumber { get { return XMLHelper.GetChildValue("ESFProjDosNumber", Node, NSMgr); } set { XMLHelper.SetChildValue("ESFProjDosNumber", value, Node, NSMgr); OnLearningDeliveryPropertyChanged(); OnPropertyChanged("ESFProjDosNumber"); } }
		public int? ESFLocProjNumber { get { string ESFLocProjNumber = XMLHelper.GetChildValue("ESFLocProjNumber", Node, NSMgr); return (ESFLocProjNumber != null ? int.Parse(ESFLocProjNumber) : (int?)null); } set { XMLHelper.SetChildValue("ESFLocProjNumber", value, Node, NSMgr); OnLearningDeliveryPropertyChanged(); OnPropertyChanged("ESFLocProjNumber"); } }
		...
		//public bool ShouldMigrateLearningDelivery
		//{
		//    get
		//    {
		//        bool bRet = false;
		...
		public bool WPL
		{
			get
			{
				return GetFAMCode(LearningDeliveryFAM.SingleOccurrenceFAMs.WPL) == "1";
			}
			set
			{
				if (value)
					SetFAM(LearningDeliveryFAM.SingleOccurrenceFAMs.WPL, "1");
				else
					RemoveFAM(LearningDeliveryFAM.SingleOccurrenceFAMs.WPL);
				OnPropertyChanged("WPL");
			}
		}
		public bool RES
		{
		...
		public string TBS
		{
			get
			{
				return GetFAMCode(LearningDeliveryFAM.SingleOccurrenceFAMs.TBS);
			}
			set
			{
				if (value != null)
					SetFAM(LearningDeliveryFAM.SingleOccurrenceFAMs.TBS, value);
				else
					RemoveFAM(LearningDeliveryFAM.SingleOccurrenceFAMs.TBS);
				OnPropertyChanged("TBS");
			}
		}
		...
		public List<ApprenticeshipTrailblazerFinancialDetails> ApprenticeshipTrailblazerFinancialDetailsList = new List<ApprenticeshipTrailblazerFinancialDetails>();
			...
			switch (NodeName)
			{
				case "LearningDeliveryFAM":
					if (LearningDeliveryWorkPlacementList.Count() == 0)
						...
						Node.InsertBefore(NewNode, ApprenticeshipTrailblazerFinancialDetailsList.First().Node);
					break;
                case "LearningDeliveryWorkPlacement":
                    if (ApprenticeshipTrailblazerFinancialDetailsList.Count() == 0)
                        AppendToLastOfNodeNamed(NewNode, "ApprenticeshipTrailblazerFinancialDetails");
                    else
                        Node.InsertBefore(NewNode, LearningDeliveryWorkPlacementList.First().Node);
                    break;
                case "ApprenticeshipTrailblazerFinancialDetails":
		...
		internal LearningDelivery(XmlNode Node, XmlNamespaceManager NSMgr)
		{
			...
			nodes = Node.SelectNodes("./ia:ApprenticeshipTrailblazerFinancialDetails", NSMgr);
			foreach (XmlNode node in nodes)
				ApprenticeshipTrailblazerFinancialDetailsList.Add(new ApprenticeshipTrailblazerFinancialDetails(node, NSMgr));
		...
		internal LearningDelivery(LearningDelivery MigrationLearningDelivery, XmlNode Node, XmlNamespaceManager NSMgr)
		{
			...
			if (MigrationLearningDelivery.AimType == 4 && MigrationLearningDelivery.HasFAM("LDM", "323"))
			{
				this.AimType = 3;
				this.ProgType = 24;
			}
			...
			this.ESFProjDosNumber = MigrationLearningDelivery.ESFProjDosNumber;
			this.ESFLocProjNumber = MigrationLearningDelivery.ESFLocProjNumber;
			...
			foreach (LearningDeliveryFAM migrationItem in MigrationLearningDelivery.LearningDeliveryFAMList)
			{
				XmlNode newNode = Node.OwnerDocument.CreateElement("LearningDeliveryFAM", NSMgr.LookupNamespace("ia"));
			...
			{
				XmlNode newNode = Node.OwnerDocument.CreateElement("ProviderSpecDeliveryMonitoring", NSMgr.LookupNamespace("ia"));
				ProviderSpecDeliveryMonitoring newInstance = new ProviderSpecDeliveryMonitoring(migrationItem, newNode, NSMgr);
				ProviderSpecDeliveryMonitoringList.Add(newInstance);
				AppendToLastOfNodeNamed(newNode, newNode.Name);
			}
		...
		public string this[string columnName]
		{
			get
			{
				...
				if (columnName == "LearnRefNumber")
				{
					if (LearnAimRef != null)
					{
						if (LearnAimRef.Length > 8)
							result = String.Format("{0} exceeds maximum length (8).", columnName);
					}
				}
				return result;
			}
