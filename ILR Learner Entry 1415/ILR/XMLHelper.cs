...
namespace ILR
{
    internal static class XMLHelper
    {
        ...
        {
            {"ApprenticeshipTrailblazerFinancialDetails",new List<string>() {"TBFinType","TBFinCode","TBFinDate","TBFinAmount"}},
            ...
            {"Learner",new List<string>() {"LearnRefNumber","PrevLearnRefNumber","PrevUKPRN","ULN","FamilyName","GivenNames","DateOfBirth","Ethnicity","Sex","LLDDHealthProb","NINumber","PriorAttain","Accom","ALSCost","PlanLearnHours","PlanEEPHours","Dest","LearnerContact","ContactPreference","LLDDandHealthProblem","LearnerFAM","ProviderSpecLearnerMonitoring","LearnerEmploymentStatus","LearnerHE","LearningDelivery"}},
            ...
            {"LearningDelivery",new List<string>() {"LearnAimRef","AimType","AimSeqNumber","LearnStartDate","OrigLearnStartDate","LearnPlanEndDate","FundModel","ProgType","FworkCode","PwayCode","PartnerUKPRN","DelLocPostCode","PriorLearnFundAdj","OtherFundAdj","ESFProjDosNumber","ESFLocProjNumber","EmpOutcome","CompStatus","LearnActEndDate","WithdrawReason","Outcome","AchDate","OutGrade","SWSupAimId","LearningDeliveryFAM","ApprenticeshipTrailblazerFinancialDetails","ProviderSpecDeliveryMonitoring","LearningDeliveryHE","LearningDeliveryWorkPlacement"}},
            {"LearningDeliveryFAM",new List<string>() {"LearnDelFAMType","LearnDelFAMCode","LearnDelFAMDateFrom","LearnDelFAMDateTo"}},
            {"LearningDeliveryHE",new List<string>() {"NUMHUS","SSN","QUALENT3","SOC2000","SEC","TOTALTS","UCASAPPID","TYPEYR","MODESTUD","FUNDLEV","FUNDCOMP","STULOAD","YEARSTU","MSTUFEE","PCOLAB","PCFLDCS","PCSLDCS","PCTLDCS","SPECFEE","NETFEE","DOMICILE","ELQ"}},
            ...
            {"LLDDandHealthProblem",new List<string>() {"LLDDType","LLDDCode"}},
            ...
            {"Source",new List<string>() {"ProtectiveMarking","UKPRN","TransmissionType","SoftwareSupplier","SoftwarePackage","Release","SerialNo","DateTime","ReferenceData","ComponentSetVersion"}},
        ...
        private static XmlNode CreateChildNode(string Name, XmlNode Node, XmlNamespaceManager NSMgr)
        {
            ...
            else
            {
                ...
                {
                    if (attributes.IndexOf(childNode.Name) > newNodeIndex)
                    {
